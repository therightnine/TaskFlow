...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow p-8">
        <div class="mb-6 flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Modifier le role</h1>
                <p class="text-sm text-gray-500 mt-1">Mettez a jour les informations et la description du role.</p>
            </div>
            <span class="inline-flex rounded-full bg-cyan-50 px-3 py-1 text-xs font-semibold text-cyan-700">ID #{{ $role->id }}</span>
        </div>
        ...
        <form method="POST" action="{{ route('admin.roles.update', $role->id) }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="5"
                    maxlength="500"
                    class="mt-2 w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary @error('description') border-red-300 focus:ring-red-400 @enderror">{{ old('description', $role->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
